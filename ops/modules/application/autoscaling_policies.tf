...
resource "aws_autoscaling_policy" "up" {
  name                   = "${aws_ecs_cluster.training_cluster.name}-scaleUp"
...
resource "aws_autoscaling_policy" "down" {
  name                   = "${aws_ecs_cluster.training_cluster.name}-scaleDown"
  scaling_adjustment     = -1
  adjustment_type        = var.adjustment_type
  cooldown               = var.policy_cooldown
  policy_type            = "SimpleScaling"
...
resource "aws_cloudwatch_metric_alarm" "scaleUp" {
  alarm_name          = "${aws_ecs_cluster.training_cluster.name}-scaleUp"
  ...
  dimensions = {
    ClusterName = aws_ecs_cluster.training_cluster.name
  }
}

resource "aws_cloudwatch_metric_alarm" "scaleDown" {
  alarm_name          = "${aws_ecs_cluster.training_cluster.name}-scaleDown"
